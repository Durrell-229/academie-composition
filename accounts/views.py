...
def dashboard_view(request):
    ...
    if role == 'eleve':
        ...
        try:
            ...
            my_referrals = Referral.objects.filter(parrain=user, is_active=True).select_related('filleul')
            referral_count = my_referrals.count()
            referral_bonus_xp = my_referrals.aggregate(Sum('bonus_xp'))['bonus_xp__sum'] or 0
            # Générer ou récupérer le code de parrainage
            my_referral_code = f"{user.matricule or user.email[:6].upper()}-{user.id.hex[:6]}".upper()
        except Exception:
            my_referrals = []
            referral_count = 0
            referral_bonus_xp = 0
        ...
        try:
            ...
            # Mon rang
            my_leaderboard = Leaderboard.objects.filter(user=user, periode='alltime').first()
            my_rank = my_leaderboard.rank if my_leaderboard else None
            my_score = my_leaderboard.score if my_leaderboard else 0
            # Top 10
            leaderboard_top10 = Leaderboard.objects.filter(periode='alltime').select_related('user').order_by('rank')[:10]
            total_players = Leaderboard.objects.filter(periode='alltime').count()
        except Exception:
            my_rank = None
            my_score = 0
            leaderboard_top10 = []
        ...
        context.update({
            ...
            'referral_bonus_xp': referral_bonus_xp,
    ...
    # ─────────────────────────────────────────
        ...
        # 2. Tendances (comparaison ce mois vs mois dernier)
        try:
            this_month = now.replace(day=1)
            last_month_end = this_month - timedelta(days=1)
            last_month_start = last_month_end.replace(day=1)

            sessions_this_month = CompositionSession.objects.filter(created_at__gte=this_month).count()
            sessions_last_month = CompositionSession.objects.filter(
        ...
        # 3. Stats anti-triche détaillées
        try:
            from compositions.models import AntiCheatLog
            cheat_events_today = AntiCheatLog.objects.filter(
                timestamp__gte=now - timedelta(days=1)
            ).count()
            cheat_events_week = AntiCheatLog.objects.filter(
                timestamp__gte=now - timedelta(days=7)
            ).count()
            most_common_cheat = AntiCheatLog.objects.values('type_event').annotate(
                count=Count('id')
            ).order_by('-count').first()
        except Exception:
            cheat_events_today = 0
            cheat_events_week = 0
            most_common_cheat = None

        # 4. Stats referral/parrainage
        try:
            from .models import Referral
            total_referrals = Referral.objects.count()
            active_referrals = Referral.objects.filter(is_active=True).count()
            total_referral_xp = Referral.objects.aggregate(Sum('bonus_xp'))['bonus_xp__sum'] or 0
            top_parrains = Referral.objects.values('parrain__first_name', 'parrain__last_name').annotate(
                count=Count('id')
            ).order_by('-count')[:5]
        except Exception:
            total_referrals = 0
        ...
        try:
            from .models import Leaderboard
            leaderboard_weekly = Leaderboard.objects.filter(periode='weekly').select_related('user').order_by('rank')[:5]
            leaderboard_monthly = Leaderboard.objects.filter(periode='monthly').select_related('user').order_by('rank')[:5]
            leaderboard_alltime = Leaderboard.objects.filter(periode='alltime').select_related('user').order_by('rank')[:5]
        except Exception:
            leaderboard_weekly = []
            leaderboard_monthly = []
            leaderboard_alltime = []
        ...
        try:
            ...
            admin_count = Bulletin.objects.filter(type_bulletin='administratif').count()
            pro_count = Bulletin.objects.filter(type_bulletin='professionnel').count()
            bulletins_mois = Bulletin.objects.filter(created_at__gte=this_month).count()
        except Exception:
            total_bulletins = 0
            admin_count = 0
            pro_count = 0
        ...
        context.update({
            ...
            'cheat_events_today': cheat_events_today,
            'cheat_events_week': cheat_events_week,
            'most_common_cheat': most_common_cheat,
